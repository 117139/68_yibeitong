<template>
	<view>
		...
		<view v-if="htmlReset==0" class="container">
			...
			<view class="goodsBox">
				<view class="goods">
					<block  v-if="datas.goods_name">
						<view class="goods1" @tap="jump" :data-url="'/pagesLzc/deatil/deatil?sku_id='+datas.sku_id">
							<view class="goodsImg">
								...
								<image class="goodsImg" :src="getimg(datas.goods_pic[0])" mode="aspectFill"></image>
							</view>
							<view class="goodsinr flex_1">
								<!-- <view class="goodsname fz30 c30 oh1">{{item.goods_name}}</view> -->
								<view class="goodsname">{{datas.goods_name}}</view>
								...
								<view class="goodspri1">
									...
									<view class="goods_pri">
										￥<text >{{datas.goods_pay_price}}</text>
									</view>
									<view>×{{datas.goods_sum}}</view>
								</view>
							</view>
						</view>
					</block>
			...
			<view class="guige_list">
			  ...
			  <view class="guige_li">
					...
					<view class="dis_flex aic guige_r">
					  <view >{{datas.customerExpect==10?'退货':datas.customerExpect==20?'换货':''}}</view>
					</view>
				</view>
			  <view class="guige_li">
					<view class="guige_l">
			  ...
			  <!-- <view  class="guige_li">
					...
					<view class="dis_flex aic guige_r">
					  <view >
							<block v-for="(item,index) in datas.logisticsArr">
								{{item +' '}} 
							</block>
						</view>
					</view>
			  ...
			  <view v-if="datas.type!=3"   class="guige_li">
				...
				<!-- <block  v-if="datas.expressCompany">
					<view class="guige_li">
						<view class="guige_l">
						  <view class="guige_l_name">用户填写的物流公司：</view>
						</view>
						<view class="dis_flex aic guige_r">
						  <view >{{datas.expressCompany}}</view>
						</view>
					</view>
					<view class="guige_li">
						<view class="guige_l">
			...
			<!-- <view class="guige_list"  v-if="datas.s_status==2&&datas.type!=1&&datas.expressCompany==''">
				
				<picker @change="bindPickerChange" data-type="wl_list" value="" :range="datas.logisticsArr"   class="set_name">
					<view class="guige_li">
						<view class="guige_l">
						  <view class="guige_l_name">物流公司：</view>
						</view>
						<view class="dis_flex aic guige_r">
						  <view>{{datas.logisticsArr[wl_idx]}}</view>
				...
				<picker @change="bindPickerChange" mode = "date" data-type="wl_time" value=""   class="set_name">
					<view class="guige_li">
						<view class="guige_l">
						  <view class="guige_l_name">发货日期：</view>
						</view>
						<view class="dis_flex aic guige_r">
						  <view>{{deliverDate}}</view>
						  <text class="iconfont iconnext-m"></text>
						</view>
					</view>
				</picker>
				<view class="guige_li">
					...
					<view class="dis_flex aic guige_r">
					  	<input class="set_int" placeholder="请填写您的售后物流的运费" type="0" v-model="wl_pri" maxlength="50"></input>
					</view>
				</view>
			</view> -->
			<view class="o_cz">
				<!-- <view v-if="datas.s_status==2&&datas.expressCompany==''"  class="qx" @tap="save_val">提交</view> -->
				<!-- <view v-if="datas.expressCompany" class="qx" @tap.stop="jump" :data-url="'/pagesA/Order_wuliu/Order_wuliu?id='+datas.id">查看物流</view> -->
			</view>
			<!-- 底部占位 -->
	...
	export default {
		...
		onLoad: function(option) {
			...
			if (option.id) {
				...
				that.id = option.id
				that.oid=option.oid
				that.o_order_sernum=option.o_order_sernum
		...
		methods: {
			...
			bindPickerChange(e){
				...
				if(datas.type=='wl_list'){
					that.wl_idx=e.detail.value
				}
				if(datas.type=='wl_time'){
					
					that.deliverDate=e.detail.value
				}
			},
			save_val() {
				/*if (!this.wl_gs) {
				...
				var datas = {
					...
					jd_o_after_sale_id: that.datas.jd_o_after_sale_id,
					expressCompany: that.datas.logisticsArr[that.wl_idx],
					expressCode: that.wl_num,
					deliverDate: that.deliverDate,
					freightMoney:that.wl_pri
				}
				if (that.btnkg == 1) {
					return
				} else {
				...
				var jkurl = '/jd.JdAfterSale/updateSendInfo'
			...
			getdata() {
				...
				var datas = {
					token: that.$store.state.loginDatas.userToken||'',
					o_order_sernum: that.o_order_sernum
				}
				// 单个请求
				service.P_get('/jd.JdAfterSale/details', datas).then(res => {
					console.log(res)
					if (res.code == 1) {
						that.htmlReset = 0
