	...
	<view>
		...
		<view class="container" v-if="htmlReset==0">
				...
				<view class="goodsBox contbox">
					<view class="goods">
						...
						<view class="goods1">
							...
							<view class="goodsImg">
								<image v-if="datas.gd_vice_pic.length>0" class="goodsImg"
									:src="getimg(datas.gd_vice_pic[0])" mode="aspectFill"></image>
								<image v-else class="goodsImg" :src="getimg(datas.gd_mastr_pic[0])" mode="aspectFill">
								</image>
							</view>
							<view class="goodsinr">
			...
			<view class="fw_list">
				...
				<picker class="w100" @change="qiChange" :value="QiIndex" :range="qiList" range-key='name'
					v-if="is_jd==1" mode=selector>
					<view class="fuwu_li bt0">
						<view class="d1">客户期望售后类型</view>
						<view class="fw_msg">
							{{qiList[QiIndex].name}}
						</view>
						<text class="iconfont iconnext-m"></text>
				...
				<picker class="w100" @change="BaoChange" :value="BaoIndex" :range="BaoList" range-key='name'
					v-if="is_jd==1" mode=selector>
					<view class="fuwu_li bt0">
						<view class="d1">包装描述</view>
						<view class="fw_msg">
							{{BaoList[BaoIndex].name}}
						</view>
						<text class="iconfont iconnext-m"></text>
					</view>
				</picker>
				...
				<view class="fuwu_li bt0" v-if="is_jd == 0">
					...
					<view class="fw_msg">
						...
						<view class="goods_num dis_flex aic">
							...
							<text v-if="cnum<datas.may_retreat_number" class="iconfont icon54"
								@tap.stop="onNum('+')"></text>
...
<script>
	...
	export default {
		...
		onLoad: function(option) {
			...
			if (option.is_jd) {
				that.is_jd = option.is_jd;
				console.log(that.is_jd)
			}
		},
		onShow() {
		...
		methods: {
			...
			conceal(param) {
				// 获取到传过来的 省 市 区 县数据
				console.log(param);
				that.province = param.province;
				that.city = param.city;
				that.county = param.area + param.street;
				// that.address = param.street;
				// 选择完毕后隐藏
				this.isPop = false;
			},
			save_val() {
				...
				if (that.is_jd == 1) {
					...
					var jkurl = 'jd.JdAfterSale/createAfsApply'
					var data = {
						...
						packageDesc: that.BaoList[that.BaoIndex].value,
						customerExpect: that.qiList[that.QiIndex].value
					}
				} else {
					console.log(this.datas)
