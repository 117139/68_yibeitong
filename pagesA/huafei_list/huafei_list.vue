		    ...
		    <view class="goodsBox contbox">
		      <view class="goods" v-for="(item,idx) in datas">
		        <view class="dianpu_tit">
		          ...
		          <text class="flex_1 ">订单号：{{item.pr_order_num}}</text>
		          <text v-if="item.pr_pay_status==1" >待支付</text>
		          <text v-if="item.pr_pay_status==2" style="color: #FF4141;">已支付</text>
		          <text v-if="item.pr_pay_status==3" style="color: #FF4141;">已退款</text>
		          <text v-if="item.pr_pay_status==4" style="color: #FF4141;">退款中</text>
		        </view>
						...
						<view class="goods1">
							...
							<view class="goodsImg">
								<image  class="goodsImg" :src="getimg(item.phone_info.yys_img)" mode="aspectFill"></image>
								<!-- <image class="goodsImg" :src="getimg(item1.photo)" mode="aspectFill"></image> -->
							</view>
							<view class="goodsinr">
								<view class="goodsname fz30 c30">{{item.phone_info.game_area}}</view>
								<view class="goodspri">充值电话:{{item.pr_recharge_phone}}</view>
								<view class="goodspri">充值状态:{{item.phone_recharge_status_value}}</view>
								<view class="goodspri">下单时间:{{filter.getDate_ymd(item.pr_create_time)}}</view>
								<view class="goodspri dis_flex aic">充值金额:<view class="goodspri1" style="margin-left: 10upx;"><text >¥</text>{{item.pr_recharge_money}}</view></view>
							</view>
						</view>
		        ...
		        <view class="o_xj">
							...
							<view>
                实付金额 <text style="font-size: 24upx;">¥</text> <text>{{item.pr_actually_paid_money}}</text>
		          </view>
