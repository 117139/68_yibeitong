	...
	<view class="minh100 container">
		...
		<view v-if="htmlReset==0">
			...
			<view class='dis_flex ju_a w100 pb40 pt20 bgfff tab_box'>
				<block v-for="(item,idx) in data_list" :key="idx">
				...
				<view class="goodsBox contbox">
					<view class="goods" v-for="(item,idx) in datas" :key="idx">
						<view class="dianpu_tit">
							...
							<text v-if="item.order.o_ddstatus==2&&item.order.o_paystatus==2"
								style="color: #FF4141;">待发货</text>
	...
	export default {
		...
		methods: {
			...
			getdatalist() {
				...
				let that = this
				var jkurl = service.IP_lzc + "jd.JdOrder/index"
				console.log(jkurl);
				var data = {
					token: that.$store.state.loginDatas.userToken || '',
